...
const EditContractPage = () => {
  ...
  useEffect(() => {
    if (contract) {
      ...
      // Set the department based on the employee's department
      const employeeDeptRel = departmentEmployees.find(de => de.employeeId === contract.employeeId);
      if (employeeDeptRel) {
        setSelectedDepartmentId(employeeDeptRel.departmentId);
      }
    } else {
      // If contract not found, navigate back to contracts list
      toast.error("Không tìm thấy hợp đồng");
      navigate('/contracts');
    }
  }, [contract, form, departmentEmployees, navigate]);
  
  // Filter employees by selected department
  const filteredEmployees = useMemo(() => {
    if (!selectedDepartmentId || selectedDepartmentId === "_all") return employees;
  ...
  useEffect(() => {
    if (selectedDepartmentId && selectedDepartmentId !== "_all") {
      const currentEmployeeId = form.getValues('employeeId');
      const employeeDept = departmentEmployees.find(de => 
        de.employeeId === currentEmployeeId && de.departmentId === selectedDepartmentId
      );
  ...
  }, [selectedDepartmentId, form, departmentEmployees]);
  ...
  return (
    ...
    >
      <div className="flex items-center mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold">Sửa hợp đồng</h1>
          <p className="text-muted-foreground">Cập nhật thông tin hợp đồng {contract.code}</p>
        </div>
      </div>
